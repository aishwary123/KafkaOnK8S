...
- If you are running external zookeeper, update the field externalZookeeper.servers in values.yaml
  with zookeeper service IP and PORT
...
## Guidelines for Secret Creation for Kafka Security Setup

 You need to decide the name you are going to give to your kafka helm deployment based upon that name you have to create keystore and truststore.

 Let's say we are giving helm deployment name as my-kafka.So, my headless service name will be "my-kafka-kafka".

Command to create Keystore and Truststore and then creating a secret
...
> **Note**: For more details on keystore generation, refer to section "Guidelines for Secret Creation for Kafka Security Setup"

## How Producer client will execute
...
To call producer client ( bootstrap server can be collection of list of all nodes i.e. my-kafka-kafka-0.my-kafka-kafka-headless, my-kafka-kafka-1.my-kafka-kafka-headless ,my-kafka-kafka-2.my-kafka-kafka-headless )


## How Consumer client will execute
...
To call consumer client ( bootstrap server can be collection of list of all nodes i.e. my-kafka-kafka-0.my-kafka-kafka-headless, my-kafka-kafka-1.my-kafka-kafka-headless ,my-kafka-kafka-2.my-kafka-kafka-headless )
